...
def factory_reset():
    ...
    # 6. Delete AI assessments, including the two files the merged one replaced
    for name in ("ai_assessment.json", "ai_roles.json", "ai_patterns.json"):
        try:
            stale = Path(__file__).parent / "data" / name
            if stale.exists():
                stale.unlink()
        except Exception as exc:
            errors.append(f"AI assessment ({name}): {exc}")
...
# Where the merged assessment lives.  The two files it replaced are still read
# on the way out, so an assessment made before the merge is not lost.
_AI_ASSESSMENT_FILE = Path(__file__).parent / "data" / "ai_assessment.json"


def _load_ai_assessments() -> dict:
    if _AI_ASSESSMENT_FILE.exists():
        return json.loads(_AI_ASSESSMENT_FILE.read_text(encoding="utf-8"))
    return {}


def _save_ai_assessments(data: dict) -> None:
    _AI_ASSESSMENT_FILE.parent.mkdir(parents=True, exist_ok=True)
    _AI_ASSESSMENT_FILE.write_text(json.dumps(data, indent=2), encoding="utf-8")


_AI_PATTERNS_FILE = Path(__file__).parent / "data" / "ai_patterns.json"


def _load_ai_patterns() -> dict:
    if _AI_PATTERNS_FILE.exists():
        return json.loads(_AI_PATTERNS_FILE.read_text(encoding="utf-8"))
    return {}
...
def _build_patterns_context(
    ...
    for m in matches:
        ...
        if not raw:
            continue
        try:
            aim_blocks.append(_json.loads(raw) if isinstance(raw, str) else raw)
        except Exception:
            continue
    ...
    if aim_blocks:
        ...
        lines.append(
            f"Crosshair placement: {_mean(_headline('preaim'))}Â° off target "
            f"(n={_sample('preaim')})"
        )
        lines.append(
    ...
    for m in matches:
        ...
        if not raw:
            continue
        try:
            util_blocks.append(_json.loads(raw) if isinstance(raw, str) else raw)
        except Exception:
            continue
    if util_blocks:
    ...
    if bought:
        ...
        lines.append(
            ...
            f"({_pct(thrown, bought)}% â€” carrying across rounds makes this approximate)"
        )
...
def _matches_with_recorded_play(matches: list[dict]) -> list[dict]:
...
def _map_scoped_rounds(maps: str) -> tuple[list[dict], list[dict]]:
    ...
    try:
        allowed = {m.strip().lower() for m in maps.split(",")}
        matches = _matches_with_recorded_play([
        ...
        for mid in [m["match_id"] for m in matches]:
            for r in get_round_stats(conn, mid):
                ej = r.get("enriched_json")
                try:
                    r["enriched"] = _json.loads(ej) if ej else {}
                except Exception:
                    r["enriched"] = {}
                rounds.append(r)
    finally:
...
# is actually good on.
_OVERALL_KEYS = ("aim", "utility", "behaviour", "maps")
...
def _build_overall_context(matches: list[dict], rounds: list[dict]) -> str:
    ...
    """
    parts = [_build_patterns_context("all maps", matches, rounds)]
...
@app.post("/api/performance/ai-assessment")
async def ai_assessment(maps: str = "", provider: str = "", model: str = ""):
    ...
    if overall:
        conn = _db()
        try:
            matches = _matches_with_recorded_play(get_all_matches(conn))
        finally:
            conn.close()
        if not matches:
        ...
        context = _build_overall_context(matches, all_rounds)
        system_prompt = _OVERALL_SYSTEM_PROMPT
        keys = _OVERALL_KEYS
        store_key = _OVERALL_KEY
    else:
        ...
        context = "\n\n".join([
            _build_patterns_context(map_display, matches, all_rounds),
            "=== ROUND-BY-ROUND POSITIONS ===",
            _build_role_context(map_display, ct_rounds, t_rounds),
        ])
    ...
    try:
        result = _json.loads(_strip_json_fences(response))
...
@app.get("/api/performance/ai-assessment")
def get_persisted_ai_assessment(maps: str = ""):
    ...
    """
    if not maps.strip():
        return _load_ai_assessments().get(_OVERALL_KEY) or {}
